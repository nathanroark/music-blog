<script lang="ts">
	...
	export const updatePanels = ({ left, right }: any) => {
		if (left !== undefined) {
			setLeft(!!left);
		}
		if (right !== undefined) {
	...
	export let leftWidth: any;
	export let rightWidth: any;
	...
	let windowWidth: any;
	let leftOpen: any;
	let leftTransitioning: any;
	let rightOpen: any;
	let rightTransitioning: any;

	const setLeft = (open: any, transition = true) => {
		leftOpen = open;
		leftTransitioning = transition;
	};

	const setRight = (open: any, transition = true) => {
		rightOpen = open;
		rightTransitioning = transition;
	};

	const onTransitionEnd = ({ propertyName }: any) => {
	...
	const setPanelStates = (transition: any) => () => {
	...
	const makeMenuStyle = (side: any, width: any, z: any) => `
		${commonStyles}
		${side}: 0;
		width: ${width};
		z-index: ${z};
	...
	$: leftMenuStyle = makeMenuStyle(
		'left',
		mobileMode ? mobilePanelWidth : leftWidth || width,
		mobileMode && leftOpen ? '3' : '2'
	);
	$: rightMenuStyle = makeMenuStyle(
		'right',
		mobileMode ? mobilePanelWidth : rightWidth || width,
		mobileMode && rightOpen ? '2' : '1'
	);

	const makeScrimStyle = (side: any, open: any, transitioning: any, color: any) => `
		${commonStyles}
	...
	$: rightScrimStyle =
		mobileMode && makeScrimStyle('right', rightOpen, rightTransitioning, scrimColor);

	$: contentLeft = mobileMode
		? (leftOpen && `calc(100% - ${scrimWidth})`) ||
		  (rightOpen && `calc(${scrimWidth} - 100%)`) ||
		  '0px'
		: leftOpen
		? leftWidth || width
		: '0px';
	$: contentWidth = mobileMode
		? '100%'
		: `calc(100% - ${leftOpen ? leftWidth || width : '0px'} - ${
				rightOpen ? rightWidth || width : '0px'
		  })`;
	$: contentStyle = `
		${commonStyles}
		left: ${contentLeft};
...
<div style="position: absolute; top: 0; bottom: 0; left: 0; right: 0; overflow-x: hidden;">
	...
	{#if $$slots.left && mobileMode}
		...
		<div style={leftScrimStyle} on:click={leftScrimOff} on:keypress={leftScrimOff} />
	{/if}

	{#if $$slots.right && mobileMode}
		<!-- svelte-ignore a11y-no-static-element-interactions -->
		<div style={rightScrimStyle} on:click={rightScrimOff} on:keypress={rightScrimOff} />
	{/if}
</div>
